...
import { getUserProducts, removeProd } from '../../../services/productService'
...
function Products({ uid, userProducts, onLoadProdsSuccess, onLoadProd, onProdRemovedSucess }) {
    ...
    const onRemove = () => {
        removeProd(id).then(_ => {
            toast.success('Produto removido com sucesso!')
            onProdRemovedSucess(id)
        }).catch(err => {
    ...
    return(
       <Grid container>
           ...
           {
               show && <ModalCustom show={show} title={title} body={body} onToggle={onToggle} onMethod={onRemove} />
           }
       </Grid>
    )
}
