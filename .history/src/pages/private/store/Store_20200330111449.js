    ...
    const getAddress = ({ target: { value } }) => {
        ...
        getCepService(value).then(resp => {
            if(resp) {
                ...
                if(!data.erro) {
                    onLoadAddress({ cep: parseInt(data.cep), address: data.logradouro})
                }
            }
        })
    }
        ...
        <>
            <Grid container spacing={1} style={{ marginTop: '15px' }}>
                ...
                <Grid item xs={5} sm={5} md={5}>
                    <Field name={'phone1'} max={11} min={10} label={'Telefone'} component={Number} />
                </Grid>
                <Grid item xs={5} sm={5} md={5}>
                    <Field name={'phone2'} max={11} min={10} label={'Telefone (Opcional)'} component={Number} />
                </Grid>
            </Grid>
