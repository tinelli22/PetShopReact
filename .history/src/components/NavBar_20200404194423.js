...
function Navbar({ user, onLogSuccess, }) {
  ...
  const onContentPrivate = () => {
    return(
      <>
        <span id={'subitem'} onClick={_ => setContent(getPrivateScheduleOptions)}>Agendar</span>
        <span id={'subitem'} onClick={_ => setContent(getPrivatePetOptions)}>Pet</span>
        <span id={'subitem'} onClick={_ => setContent(getPrivateStoreOptions)}>Loja</span>
        <span id={'subitem'} onClick={_ => setContent(getPrivateProductOptions)}>Produto</span>
        <span id={'subitem'} onClick={_ => setContent(getPrivateAccountOptions)}>Conta</span>
      </>
    )
  }
